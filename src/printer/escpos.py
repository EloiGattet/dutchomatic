...
class EscposPrinter(Printer):
    ...
    def separator(
        ...
        """
        Imprime un séparateur avec des caractères Unicode continus.
        Utilise une font custom (Roboto) pour supporter les caractères Unicode.
        
        Args:
            char: Caractère de séparation ('-', '─', '=', '═', '_', '━')
            width_chars: Largeur en caractères (défaut: chars_per_line)
            double: Si True, imprime deux lignes
            font_path: Chemin vers la font à utiliser (défaut: default_font_path ou font système)
        """
        if not width_chars:
        ...
        unicode_chars = {
            ...
            "_": "━",      # U+2501 Box Drawings Heavy Horizontal
            "─": "─",
            "═": "═",
            "━": "━",
        }
        
        # Utiliser le caractère Unicode correspondant
        unicode_char = unicode_chars.get(char, "─")
        ...
        # On crée d'abord un test pour mesurer la largeur d'un caractère
        if PIL_AVAILABLE and font_to_use:
            try:
                test_font = self._load_font(font_size, font_to_use)
                if test_font:
                    # Mesurer la largeur d'un caractère Unicode
        ...
        # On répète suffisamment pour couvrir toute la largeur
        num_chars = max(200, int(target_width_px / 8))  # Au moins 200 caractères ou assez pour la largeur
    ...
    def print_text(self, text: str, header_images: Optional[list] = None, bonus_images: Optional[list] = None, city_images: Optional[list] = None) -> bool:
        ...
        try:
            ...
            for line in lines:
                ...
                # Imprimer la ligne
                # Détecter si la ligne contient des emojis
